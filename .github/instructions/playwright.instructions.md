...
> [!NOTE]
> This file covers how specs should be written. To *run* the E2E suite, use the `quality-checks` skill (if avaialble) — never invoke `npx playwright test` directly.

1. **Run**: Execute the suite through the `quality-checks` skill (if avaialble).
