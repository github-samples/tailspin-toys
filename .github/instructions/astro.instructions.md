...
- Document each reusable component or layout's `Props` interface and each declared prop with TSDoc/JSDoc so its contract, defaults, and optional behavior are clear
...
- Comment intent, constraints, and non-obvious decisions; do not restate code mechanics (see `coding-standards.instructions.md`)
